...
trail() { for t in job.events.v1 credit.events.v1 llm.events.v1; do docker exec llm-mcp-kafka-1 /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:19092 --topic $t --from-beginning --property print.headers=true --timeout-ms 3000 2>/dev/null | grep "$1" | grep -oE 'eventType:[A-Za-z]+' | sed "s/eventType:/  $t  /"; done; }
