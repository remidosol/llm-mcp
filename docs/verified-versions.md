...
| Artifact Registry image data source | `google_artifact_registry_docker_image` (provider-google 20.3.1; Java class `DataGoogleArtifactRegistryDockerImage`): args `location`, `repository_id`, `image_name` (`name`, `name:tag` or `name@digest`; no tag = latest modified), attribute `self_link` = pullable URI with `@sha256:` digest | terraform-provider-google docs (`website/docs/d/artifact_registry_docker_image.html.markdown`), 2026-09-14 |
| PR plan comments | `borchero/terraform-plan-comment@v2`: inputs `token`, `planfile`, `working-directory`, `terraform-cmd` (`tofu`), `header`, `skip-empty`; `buck2 build --show-simple-output //infra:plan@<stack>` prints the `tofu.plan` path whose directory holds `.terraform` from `init@` | github.com/borchero/terraform-plan-comment README, 2026-09-14 |
